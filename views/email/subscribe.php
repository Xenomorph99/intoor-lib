<?php
/**
 * HTML email template sent to users the first time they subscribe to the mailing list.
 *
...
<tbody style="<?= $reset ?>" role="rowgroup">
	...
	<tr class="banner" style="<?= $reset ?>">
		<td style="background-color:#<?= $color['banner'] ?>; line-height:0; <?= $reset ?>" colspan="3">
			<img src="<?= get_option( $prefix . 'subscribe_banner' ) ?>" alt="" width="600" height="<?= get_option( $prefix . 'subscribe_banner_height' ) ?>" role="presentation" style="<?= $reset ?>" ondragstart="return false;">
		</td>
	</tr>
	<?php endif; ?>
							...
							<?php endif; ?>
								...
								<td style='line-height:0; margin:0; padding:20px 0; vertical-align:top; width:150px;'>
									<span style="display:block; overflow:hidden; width:150px; height:150px; <?= $reset ?> -webkit-border-radius:75px; -moz-border-radius:75px; -o-border-radius:75px; -ms-border-radius:75px; border-radius:75px;">
										<a href="<?= get_permalink( $post->ID ) ?>" role="link" style="display:block; <?= $reset ?>">
								...
								<?php endif; ?>
									<h2 style="font:100 20px/24px <?= $helvetica ?>; margin:0 0 4px 0; padding:0;" role="heading" aria-level="1"><a href="<?= get_permalink( $post->ID ) ?>" role="link" style="color:#<?= $color['text_heading'] ?>; text-decoration:none; <?= $reset ?>"><?= $post->post_title ?></a></h2>
